Function Start-Application {
    ...
    if ($Index) {
        ...
        $AppIndex = Get-ChildItem $path -include *.exe, *.msc -Recurse | Where-Object { $_.FullName -notlike '*en-us*' } | Select-Object FullName, PSChildName
        $ErrorActionPreference = 'continue'

        $AppIndex | Export-Clixml $env:USERPROFILE\Documents\ApplicationIndex.xml
    ...
    #Launch an application
        ...
        if (Test-Path $env:USERPROFILE\Documents\ApplicationIndex.xml) {
            ...
            elseif ($Program.FullName.count -gt '1' ) {
                ...
                $Selection = $Program.FullName | Out-GridView -PassThru -Title "Select Your desired Applicaiton to Open"
        ...
        #If the index doesn't exist, warn to build it.
            ...
            Write-Host
            Write-Warning "No index file found. First build the index using: Start-Applicaiton -index"
        }
    }
}